...
export default function TelemetryPanel(props: { selected: Site | null; onLiveView?: (site: Site) => void }) {
  const { selected, onLiveView } = props;
        ...
        {!selected ? (
          ...
          <>
            ...
            <div className="rows" style={{ marginTop: 10 }}>
              <div className="row"><span>Location</span><b>{selected.city}, {selected.region} • {selected.country}</b></div>
              {selected.category === "lev" && selected.ownerName && (
                <div className="row"><span>Owner</span><b>{selected.ownerName}</b></div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
              <button
                type="button"
                className="cta"
                onClick={() => onLiveView?.(selected)}
                title="Open the simulated live camera view for this node"
              >
                Go to Live View
              </button>
            </div>
            ...
            <div className="rows">
              <div className="row"><span>Online since</span><b>{t ? fmtDuration(Date.now() - t.startedAt) : "—"}</b></div>
              ...
              <div className="row"><span>Distance traveled</span><b>{t && typeof (t as any).distanceKm === 'number' ? fmt((t as any).distanceKm, 1) : "—"} km</b></div>
              <div className="row"><span>Battery life</span><b>{t && typeof (t as any).batteryPct === 'number' ? fmt((t as any).batteryPct, 0) : "—"}%</b></div>
