...
function makeBase(site: Site): Telemetry {
  ...
  return {
    ...
    batteryPct: clamp(92 + (Math.random()*2 - 1) * 6, 12, 100),
    distanceKm: Math.max(0, 200 + Math.random() * 4200),
      ...
      setState((prev) => {
        ...
        for (const site of sitesRef.current) {
          ...
          next[site.id] = {
            ...
            tempC,
            energyKwhToday,
            batteryPct: clamp(t.batteryPct - (site.category === "battery" ? 0.008 : 0.015) + (Math.random() * 0.03), 5, 100),
            distanceKm: t.distanceKm + ((site.category === "lev" || site.category === "humanoid") && isOnline ? (speedKph / 3600) * Math.min(1, (now - t.lastUpdateAt)/1000) : 0),
