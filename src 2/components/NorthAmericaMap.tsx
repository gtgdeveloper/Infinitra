...
function iconForSite(site: Site){
  if (site.category === "humanoid") return "/humanoid.jpg";
  if (site.category === "lev") {
    const p = (site.product || "").toLowerCase();
    // Specific LEV product icons
    if (p.includes("leggera")) return "/leggera.jpg";
    if (p.includes("tectus") || p.includes("mobility") || p.includes("scooter")) return "/tectusmobility.jpg";
    // Generic / Terra / other e-bike fleet
    return "/ebiketera.jpg";
  }
  // battery stays as existing svg marker
  return "";
}


function makeDivIcon(site: Site, levFlash: boolean, batteryFlash: boolean, humanoidFlash: boolean) {
  const category = site.category;
  const dotClass =
    category === "lev" ? "dot-lev" : category === "battery" ? "dot-battery" : "dot-humanoid";

  const flashClass =
    (category === "lev" && levFlash) ||
    (category === "battery" && batteryFlash) ||
    (category === "humanoid" && humanoidFlash)
      ? " levFlash"
      : "";

  const imgSrc = iconForSite(site);
  ...
  const core =
    category === "battery"
      ? svg
      : `<img class="markerImg" src="${imgSrc}" alt="${category}" />`;

  const html = `
    <div class="markerIcon${flashClass}">
      ${core}
...
export default function NorthAmericaMap(props: {
  ...
  const { onSelect, filters, onFiltersChange, levFlash = false, batteryFlash = false, humanoidFlash = false } = props;
        ...
        <MapContainer bounds={bounds} maxBounds={bounds.pad(0.1)} minZoom={3} maxZoom={8} scrollWheelZoom worldCopyJump>
          ...
          {visibleSites.map((site) => (
            <Marker
              ...
              icon={makeDivIcon(site, levFlash, batteryFlash, humanoidFlash)}
